...
export  const Navbar = () => {
  ...
  return (
    <Row className="w-full  p-4  px-6 justify-between h-[70px] mt-2 items-center mx-auto xl:w-[1224px]  ">

        {/* this is for the logo */}
        <Col className="w-[50px] h-full items-start justify-center  ">
        <Image src={logo} alt="logo " width={100} />
        </Col>

        {/* this is for the buttons components */}

        <Row className="gap-x-8 hidden md:flex">
        <Button onClick={() => router.push("/auth/login")} className=" text-lg text-white hover:bg-white hover:text-black rounded-full "> 
        log in
        </Button>
        <Button onClick={() => router.push("/auth/register")} className=" bg-orange-500 text-white rounded-full text-xl hover:bg-orange-600 cursor-pointer "> 
        sign up
        </Button>
   
        </Row>
        <Col className=" md:hidden">
        <svg width="40" height="34" viewBox="0 0 20 14" fill="none" xmlns="http://www.w3.org/2000/svg">
